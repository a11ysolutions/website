...
function Form() {
  ...
  const onSubmit = (event) => {
    ...
    if (!token && !hasOtherErrors) {
      const recaptchaError = 'Please complete the CAPTCHA, and verify you are not a robot.'
    ...
    if (Object.keys(validationErrors).length === 0 && token) {
      ...
      emailjs
        ...
        .then((response) => {
          setIsSubmit(false)
          setAlert({})
          setAlert({ message: 'Submission Successful! We will be in touch soon.', type: 'success' })
  ...
  return (
    <div style={{ marginTop: '4em' }}>
      <form ref={form} onSubmit={onSubmit} noValidate className="form-inputs-container">
        ...
        <div>
          ...
          />
          <div id="error-recaptcha" className="error-message" aria-live="polite">
